...
codeunit 60083 "DXR MCC DXP Migr Phase4"
{
    ...
    // - copies the 9 pre-DXR legacy tables ("DX Payment Setup" etc., an ID-collision-driven
    ...
    Permissions = tabledata "DX Payment Setup" = RIM,
    ...
    local procedure CopyPaymentSetupFields(Source: Record "DX Payment Setup"; var Dest: Record "DX Payment Setup"; IncludePrimaryKey: Boolean)
    begin
    ...
    local procedure CopyPromotionBinCardFields(Source: Record "DX Promotion Bin Card"; var Dest: Record "DX Promotion Bin Card"; IncludePrimaryKey: Boolean)
    begin
        if IncludePrimaryKey then
    ...
    local procedure CopyStorePaymentsFields(Source: Record "Store Payments DX"; var Dest: Record "Store Payments DX"; IncludePrimaryKey: Boolean)
    var
    ...
    local procedure CopyPaymentProcessLogFields(Source: Record "Payment Process Logs"; var Dest: Record "Payment Process Logs"; IncludePrimaryKey: Boolean)
    begin
        if IncludePrimaryKey then
    ...
    local procedure CopyPromoBinHeaderFields(Source: Record "DX Promo Bin Header"; var Dest: Record "DX Promo Bin Header"; IncludePrimaryKey: Boolean)
    begin
        if IncludePrimaryKey then
            Dest."No." := Source."No.";
    ...
    local procedure CopyPromotionBinItemFields(Source: Record "DX Promotion Bin Items Lines"; var Dest: Record "DX Promotion Bin Items Lines"; IncludePrimaryKey: Boolean)
    begin
        if IncludePrimaryKey then begin
    ...
    local procedure CopyPromotionBinLineFields(Source: Record "DX Promotion Bin Lines"; var Dest: Record "DX Promotion Bin Lines"; IncludePrimaryKey: Boolean)
    begin
        if IncludePrimaryKey then begin
            Dest."Offer No." := Source."Offer No.";
    ...
    local procedure CopyPromotionBinSetupFields(Source: Record "DX Promotion Bin Setup"; var Dest: Record "DX Promotion Bin Setup"; IncludePrimaryKey: Boolean)
    begin
        if IncludePrimaryKey then
            Dest."Key" := Source."Key";
        Dest.Active := Source.Active;
    ...
    // local procedure CopyErrorAuditLogFields(Source: Record "DX Error Audit Log"; var Dest: Record "DX Error Audit Log"; IncludePrimaryKey: Boolean)
    // begin
    //     if IncludePrimaryKey then
    //         Dest."Entry No." := Source."Entry No.";
