codeunit 60021 "DXR MCC Adapt BC Phase1"
{
    ...
    // behavior exactly (raw Codeunit.Run(54858) also invokes OnRun only). Unlike SD/DXP, Base
    // Controls' migration codeunits were already confirmed correct/idempotent as-is (Discovery:
    // "0 confirmed bugs") - this change is purely the invocation mechanism, no behavior change.
    trigger OnRun()
    var
