...
## Addendum (2026-08-24, discovered while writing the implementation plan)

- **`Access = Internal` blocks direct typed `Record` access for most of DRLOC's `DXR_*` tables** (confirmed: `CHANGELOG.md:25` — "All fiscal codeunits and tables of the DR Localization... are now declared Access = Internal"; MCC's app ID `a5b9bf50-7945-4455-8df4-3be9c7431a7b` is confirmed NOT in DRLOC's `internalsVisibleTo`). For these tables, MCC cannot declare `Record "DXR_X"` itself. Resolution: a **thin-wrapper adapter shape** — the typed, direct-field copy logic lives inside a public procedure on DRLOC's OWN (non-Internal) migration codeunit, and MCC's adapter is a real `OnRun` that calls just that one procedure. This still satisfies "zero RecordRef on MCC's side" and, once DRLOC's own procedure is rewritten off RecordRef too, "zero RecordRef end-to-end." Confirmed NOT an issue for BELLON (1 isolated `Access = Internal` match in its whole source, not pervasive) — direct Record access applies there as originally designed. Every other extension gets this same audit before its first adapter is written (implementation plan's Task 0.2).
- **Two additional legitimate `RecordRef` exemptions found**, alongside `DXR MCC Fallback Migrator`: `DXR MCC Counter`'s generic row counter (must operate on arbitrary table IDs by design, feeds the Gap Report) and `DXR MCC Upgrade Tag Mgt`'s access to platform table 9999 "Upgrade Tags" (that table is `Access = Internal` to the System Application itself — no extension can reference it by typed `Record`, confirmed by a compiler error cited in that file's own comments). Neither is migration business logic; both stay as-is.
- **SD (Special Dispatch) is already fully converted** — confirmed zero `RecordRef`/`FieldRef` across all 8 files in `src/Adapters/SD/`, dated 2026-08-23. Not a "Discovery Summary ~80% already typed" claim carried over unverified from the superseded 2026-08-23 plan — independently re-confirmed by direct read during this plan's authoring. DXP's Phase2 adapter, by contrast, was found to still route through a generic `RecordRef`/`FieldRef` helper despite already knowing every real field name in its own source — this is exactly why every "already done" claim gets re-verified per file before being trusted, not assumed from either document.
