...
| DESB (Despacho Base) | 1 of 113 target tables: `DXRDespachoPhase1RdyBuf` (a Phase-1 readiness buffer table, isolated) | Yes — `DXR_Despacho Migr Dispatcher` (53669) is Access=Internal | **Direct** for 112 of 113 tables (overwhelming majority — same class as the confirmed BELLON baseline). One exception flagged (buffer table) with no public entry currently, low priority since it's a working buffer, not user data |
